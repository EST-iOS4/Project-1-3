                ...
                ScrollViewReader { proxy in
                    List {
                        Chart(chartData, id: \.id) { element in
                            SectorMark(angle: .value("Usage", element.count), angularInset: 1.0)
                        ...
                        ForEach(filteredItems, id: \.id) { item in
                            NavigationLink(destination: EditView(getDate: item.createdAt, getFontSize: getFontSize, editViewModel: EditViewModel(diary: item, dataManager: DiaryDataManager(context: context)))) {
                                HStack {
